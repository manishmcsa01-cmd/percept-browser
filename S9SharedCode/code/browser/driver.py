        ...
        "actions": {
            ...
            "items": {
                ...
                "properties": {
                    "type": {
                        "type": "string",
                        "enum": ["click", "type", "key", "scroll", "drag", "wait", "done", "back"],
                    },
                    "mark":   {"type": "integer"},
                    "value":  {"type": "string"},
                    "clear":  {"type": "boolean"},
...
SYSTEM_PROMPT_VISION = (
    ...
    "CRITICAL RULES:\n"
    "  - For any action requiring a 'mark' (like click, type), you MUST specify the integer id from the legend (e.g. if the legend lists '[36]<a>Text Generation</a>', specify 'mark': 36). Never use the element's name, text, or tag as the mark.\n"
    ...
    "If the information requested by the user's goal (e.g., model names, likes, downloads, prices, plan details) "
    "is already visible in the list summaries on the main search results page, do NOT click into individual item "
    "detail pages. Just read the info directly from the list page and call done(success=true, note=...) immediately "
    "to finish. Detail pages are slow and reset active filters.\n"
    "DO NOT sort or filter search results (e.g. by price low-to-high) unless the goal explicitly requires it. Sorting often ruins relevance and shows cheap accessories instead of the main products.\n"
    "DO NOT click on items, prices, or ratings just to extract them. If the data is visible in the screenshot, you ALREADY have it. Calling `done()` is enough to pass the current state to the extraction engine."
)
...
SYSTEM_PROMPT_A11Y = (
    ...
    "CRITICAL RULES:\n"
    "  - For any action requiring a 'mark' (like click, type), you MUST specify the integer id from the legend (e.g. if the legend lists '[36]<a>Text Generation</a>', specify 'mark': 36). Never use the element's name, text, or tag as the mark.\n"
    "  - DO NOT CLICK INTO ITEM/MODEL DETAIL PAGES (e.g. do not click links like 'sapientinc/HRM-Text-1B'). The main search results list page already displays the names, tasks, likes, and details of all items. Clicking into a detail page is a failure because going back in history resets all your active search/filter settings. Read the information directly from the list page and call done(success=true, note=...) immediately to finish.\n"
    ...
    "to finish. Detail pages are slow and reset active filters.\n"
    "  - DO NOT sort or filter search results (e.g. by price low-to-high) unless the goal explicitly requires it. Sorting often ruins relevance and shows cheap accessories instead of the main products.\n"
    "  - DO NOT click on items, prices, or ratings just to extract them. If the data is visible in the legend, you ALREADY have it. Calling `done()` is enough to pass the current state to the extraction engine.\n"
...
class StepRecord:
    ...
    url: str = ""
    title: str = ""
    a11y_summary: list[str] = field(default_factory=list)
...
class BaseDriver:
    ...
    async def step(self, turn: int) -> tuple[bool, bool, str]:
        ...
        try:
            ...
            rec = StepRecord(turn, "", [], f"error: decide exception: {e}",
                             self.config.provider or "unknown", self.config.model or "unknown", 0, 0, 0,
                             url=self.page.url, title=getattr(self, "title", ""), a11y_summary=[])
            self.steps.append(rec)
            return False, False, f"decide exception: {e}"
        ...
        try:
            ...
            title = ""
        a11y_summary = [e.legend_line() for e in snap.elements]
        ...
        if not parsed:
            rec = StepRecord(turn, "", [],
                             ...
                             url=url, title=title, a11y_summary=a11y_summary)
            self.steps.append(rec)
        ...
        for i, a in enumerate(actions):
            ...
            is_dropdown = False
            mark_val = a.get("mark")
            if mark_val is not None and a.get("type") in ("click", "type"):
                try:
                    el = snap.by_id(int(mark_val))
                    if el:
                        el_name = (el.name or "").strip()
                        if el_name.endswith("▾") or el_name.endswith(":") or el_name.startswith("Sort"):
                            is_dropdown = True
                except Exception:
                    pass
            try:
    ...
    async def run(self) -> DriverResult:
        ...
        for turn in range(1, self.config.max_steps + 1):
            if turn > 1:
                # Spacer delay to prevent hitting TPM limits (6,000 TPM on Groq)
                print(f"[driver] pausing 2s before Turn {turn} to respect provider TPM limits...")
                await asyncio.sleep(2.0)
