    ...
    for inp in node_inputs:
        if inp == "USER_QUERY":
            ...
            if isinstance(upstream, AgentResult):
                output_data = upstream.output
                if upstream.agent_name == "browser" and isinstance(output_data, dict):
                    output_data = {
                        k: v for k, v in output_data.items()
                        if k not in ("page_states", "screenshots", "actions")
                    }
                out.append({"id": inp, "kind": "upstream",
                            "skill": upstream.agent_name, "output": output_data})
            else:
...
def parse_skill_json(text: str) -> dict:
    ...
    print(f"[debug parse_skill_json] RAW TEXT:\n{text}\n---")
    t = (text or "").strip()
    if t.startswith("```"):
        t = t.strip("`")
        t = t.split("\n", 1)[1] if "\n" in t else t
