  ...
  browser            fetch / interact with a SPECIFIC URL through a
                     ...
                     a11y → vision). PREFER this over researcher when:
                       ...
                       - recency matters ("this week", "today",
                         ...
                         site-native sort.
                     NEVER use `browser` for `google.com`, `bing.com`,
                     or other search engines. The browser skill cannot
                     operate search engines. If you need to search the
                     web, use `researcher`.
                     ...
                     IMPORTANT: If the target site requires a search to reach the listing (e.g. Flipkart, Amazon), you MAY pass a pre-filled search URL (e.g. `https://www.flipkart.com/search?q=laptops+under+80000`). If the site has interactive filter widgets on a base directory (e.g. HuggingFace models), pass the base URL and describe the filters in `goal`.
...
IMPORTANT: You MUST include "USER_QUERY" in the `distiller` node's `inputs` so it knows what fields to extract!
...
Scoping a worker — IMPORTANT:
  ...
  - IMPORTANT: The `formatter` MUST ALSO list the `n:<browser_id>` in its inputs (in addition to the distiller) so it can see the rich comparison tables and extracted reports generated by the browser skill!
...
Example — browser usage for a specific interactive site:
{"rationale": "Use browser to fetch top items, then format.",
 "nodes": [
   {"skill":"browser","inputs":[],
    "metadata":{"label":"b1","url":"https://example.com","goal":"filter and extract"}},
   {"skill":"distiller","inputs":["USER_QUERY","n:b1"],
    "metadata":{"label":"d1"}},
   {"skill":"formatter","inputs":["USER_QUERY","n:d1","n:b1"],
    "metadata":{"label":"out"}}]}
